...
test('it retrieves routes', function () {
    ...
    expect($this->endpoint->retrieve('route-id', $query))->toBe(['data' => ['id' => 'route-id']]);
});
...
test('it verifies inbound domains', function () {
    ...
    expect($this->endpoint->verifyInboundDomain('route-id'))->toBe(['data' => []]);
});
