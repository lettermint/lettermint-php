...
test('it creates domains', function () {
    ...
    expect($this->endpoint->create($data))->toBe(['data' => ['id' => 'domain-id']]);
});
...
test('it retrieves domains', function () {
    ...
    expect($this->endpoint->retrieve('domain-id', $query))->toBe(['data' => ['id' => 'domain-id']]);
});
...
test('it deletes domains', function () {
    ...
    expect($this->endpoint->delete('domain-id'))->toBe(['deleted' => true]);
});
...
test('it verifies all dns records', function () {
    ...
    expect($this->endpoint->verifyDnsRecords('domain-id'))->toBe(['data' => []]);
});
...
test('it verifies a specific dns record', function () {
    ...
    expect($this->endpoint->verifyDnsRecord('domain-id', 'record-id'))->toBe(['data' => []]);
});
...
test('it updates domain projects', function () {
    ...
    expect($this->endpoint->updateProjects('domain-id', $data))->toBe(['data' => []]);
});
